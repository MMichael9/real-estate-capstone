...
contract Ownable {
    ...
    {
        _owner = msg.sender;
        emit OwnershipTransfer(msg.sender, address(0));
    }
    ...
    {
        require( newOwner != address(0), "New owner cannot have an empty address" );
        address prevOwner = _owner;
        _owner = newOwner;
        emit OwnershipTransfer(_owner, prevOwner);
    }

    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event OwnershipTransfer( address to, address from );
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require( (to != ownerOf(tokenId)), "Potential Owner already owns this token");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require( ( (msg.sender == owner()) || (isApprovedForAll(owner(), msg.sender) ) ), "Must be authorized to call this function" );
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require( from == ownerOf(tokenId), "sender does not own the token");

        // TODO: require token is being transfered to valid address
        require( to != address(0), "address token is being transfered to is not valid");
...
contract CustomERC721Token is ERC721Metadata("Michael RealEstate", "MIKE", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") 
{
